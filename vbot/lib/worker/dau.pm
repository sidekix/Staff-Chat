...
use lib::config;
use constant DAU_URI => 'http://www.daujones.com/daubeitraege.rss';
...
sub setup {
  ...
  unless ($child_pid) { # i am child
    while(1) {
      $0='vBot Dau worker (active)';

      # get and parse RSS feed
      if (my $feed = get(DAU_URI)) {
      ...
      for (my $i = 10; $i > 0; $i--) {
        $0="vBot Dau worker ...zzzZZZ($i min)";
        sleep 60;
      }
    }
