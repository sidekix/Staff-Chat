...
use lib::config;
use constant IBASH_URI => 'http://www.ibash.de/neueste-zitate.xml';
...
sub setup {
  ...
  unless ($child_pid) { # i am child
    while(1) {
      $0='vBot iBash worker (active)';

      # get and parse RSS feed
      if (my $feed = get(IBASH_URI)) {
      ...
      for (my $i = 10; $i > 0; $i--) {
        $0="vBot iBash worker ...zzzZZZ($i min)";
        sleep 60;
      }
    }
