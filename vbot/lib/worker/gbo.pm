...
use lib::config;
use constant GBO_URI => 'http://feeds2.feedburner.com/gbo-zitate?format=xml';
...
sub setup {
  ...
  unless ($child_pid) { # i am child
    while(1) {
      $0='vBot GBO worker (active)';

      # get and parse RSS feed
      if (my $feed = get(GBO_URI)) {
      ...
      for (my $i = 10; $i > 0; $i--) {
        $0="vBot GBO worker ...zzzZZZ($i min)";
        sleep 60;
      }
    }
