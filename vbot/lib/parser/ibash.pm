...
sub parse {
  ...
  return 1 if ($self->ibash($conn, $event));
...
sub ibash {
  ...
  return 0 unless ($message =~ /^!ibash$/i);

  # get and parse RSS feed
  my $feed = get(IBASH_URI);
...
sub help {
  ...
  if ($message =~ /^!hilfe\s*$/i) {
    push @text, "!hilfe ibash     - Forum und aktuelle Threads";
  }
  elsif ($message =~ /^!hilfe\s+ibash\b$/i) {
    push @text, "iBash-Kommandos:";
    push @text, "  !ibash          - Link zum letzten Posting";
  }
  else {
    return 0;
  }
